...
let buffH = 40;
let buffW = 25;
let generateBuffTimer = 0;
let _buffGenTime = 10;

let blocks = [[canvas.width/2 - 350/2,canvas.height - 150 - groundH,350,30]];
...
//defVal
let jumpHeight1Def = jumpHeight1;
let jumpSpeed1Def = jumpSpeed1;
let speed1Def = speed1;
let w1Def = w1;
let h1Def = h1;

let jumpHeight2Def = jumpHeight2;
let jumpSpeed2Def = jumpSpeed2;
let speed2Def = speed2;
let w2Def = w2;
let h2Def = h2;
...
let drawBuffs = () => {
    if(buff !== 0){
        switch (buff) {
            case 1:
                ctx.fillStyle = '#ff0419';
                if(x1 + w1 >= canvas.width/2 - buffW/2 && x1 < canvas.width/2 + buffW/2 && y1 + h1 <= blocks[0][1] && y1 + h1 > blocks[0][1] - buffH){
                    buff = 0;
                    speed1 *= 2;
                    setTimeout(()=>{speed1 = speed1Def},8000);
                }
                if(x2 + w2 >= canvas.width/2 - buffW/2 && x2 < canvas.width/2 + buffW/2 && y2 + h2 <= blocks[0][1] && y2 + h2 > blocks[0][1] - buffH){
                    buff = 0;
                    speed2 *= 2;
                    setTimeout(()=>{speed2 = speed2Def},8000);
                }
                break;
            case 2:
                ...
                if(x1 + w1 >= canvas.width/2 - buffW/2 && x1 < canvas.width/2 + buffW/2 && y1 + h1 <= blocks[0][1] && y1 + h1 > blocks[0][1] - buffH){
                    ...
                    jumpHeight1 *= 2;
                    jumpSpeed1 *=2;
                    setTimeout(()=>{jumpHeight1 = jumpHeight1Def;jumpSpeed1 = jumpSpeed1Def;},5000);
                }
                if(x2 + w2 >= canvas.width/2 - buffW/2 && x2 < canvas.width/2 + buffW/2 && y2 + h2 <= blocks[0][1] && y2 + h2 > blocks[0][1] - buffH){
                    buff = 0;
                    jumpHeight2 *= 2;
                    jumpSpeed2 *=2;
                    setTimeout(()=>{jumpHeight2 = jumpHeight2Def;jumpSpeed2 = jumpSpeed2Def;},5000);
                }
                break;
            case 3:
                ...
                switch (rn) {
                    ...
                    case 2:
                        ...
                        break;
                    case 3:
                        ctx.fillStyle = '#ff55d8';
                        break;
                    case 4:
                        ctx.fillStyle = '#ff6557';
                        break;
                    case 5:
                        ctx.fillStyle = '#fffe5e';
                        break;
                    case 6:
                        ctx.fillStyle = '#67ff70';
                        break;
                }
                if(x1 + w1 >= canvas.width/2 - buffW/2 && x1 < canvas.width/2 + buffW/2 && y1 + h1 <= blocks[0][1] && y1 + h1 > blocks[0][1] - buffH){
                    buff = 0;
                    ...
                    h1 *= 2;
                    setTimeout(()=>{
                        jumpHeight1 = jumpHeight1Def;
                        jumpSpeed1 = jumpSpeed1Def;
                        speed1 = speed1Def;
                        w1 = w1Def;
                        h1 = h1Def;
                        },3000);
                ...
                if(x2 + w2 >= canvas.width/2 - buffW/2 && x2 < canvas.width/2 + buffW/2 && y2 + h2 <= blocks[0][1] && y2 + h2 > blocks[0][1] - buffH){
                    ...
                    h2 *= 2;
                    setTimeout(()=>{
                        jumpHeight2 = jumpHeight2Def;
                        jumpSpeed2 = jumpSpeed2Def;
                        speed2 = speed2Def;
                        w2 = w2Def;
                        h2 = h2Def;
                    },3000);
        ...
        ctx.fillRect(canvas.width/2 - buffW/2, blocks[0][1] - buffH, buffW, buffH);
        ctx.strokeRect(canvas.width/2 - buffW/2 + 1, blocks[0][1] - buffH + 1, buffW - 1, buffH - 1);
    }
};
let loadingUlt1 = () =>{
...
let gameLoop = ()=> {
    ...
    let win1 = ()=>{
        if(!immortality2) {
            ...
            spr1 = false;
            spr2 = false;
            if ((+p1Wins.innerHTML || 1) / (+p2Wins.innerHTML || 1) >= 3) {
                speed2 = 12;
                jumpHeight2 = gravitation * 60;
                jumpSpeed2 = gravitation * 2;
            }
            if ((+p2Wins.innerHTML || 1) / (+p1Wins.innerHTML || 1) < 3) {
                speed1 = speed1Def;
                jumpHeight1 = jumpHeight1Def;
                jumpSpeed1 = jumpSpeed1Def;
            }
    ...
    let win2 = () =>{
        if(!immortality1) {
            ...
            if ((+p2Wins.innerHTML || 1) / (+p1Wins.innerHTML || 1) >= 3) {
                speed1 = 12;
                jumpHeight1 = gravitation * 60;
                jumpSpeed1 = gravitation * 2;
            }
            if ((+p1Wins.innerHTML || 1) / (+p2Wins.innerHTML || 1) < 3) {
                speed2 = 8;
                jumpHeight2 = gravitation * 50;
                jumpSpeed2 = gravitation;
            }
        }
    };
    ...
    //movement
    let cdWait1 = 0;
    if(keysDowned.indexOf('KeyD') !== -1 && (x1 >= x2 + w2 || x1 + w1 <= x2 || y1 + h1 <= y2 || y1 > y2 + h2) && x1 + w1 < canvas.width + 50){
    ...
    if(keysDowned.indexOf('KeyA') !== -1 && (x1 >= x2 + w2 || x1 + w1 <= x2 || y1 + h1 <= y2 || y1 > y2 + h2) && x1 > -50){
        if(spr1&&spr1Used < sprExpected){
            spr1Used++;
            immortality2 = true;
            immortality1 = true;
    ...
    if(keysDowned.indexOf('KeyW') !== -1 && y1 >= canvas.height - p1ground - h1){
        let p1grDef = p1ground;
        let jumpAnim = setInterval(() => {
            isJumping1 = true;
            y1 -= jumpSpeed1 / 3;
            if (y1 <= canvas.height - jumpHeight1 - p1grDef || y1 < 0) {
                falling1 = true;
                isJumping1 = false;
    ...
    if(keysDowned.indexOf('KeyS') !== -1){
        if(spr1Used < sprExpected) {
            spr1 = true;
        }
    }
    if(keysDowned.indexOf('Space') !== -1){
    ...
    if(keysDowned.indexOf('Numpad6') !== -1 && (x2 >= x1 + w1 || x2 + w2 <= x1 || y2 + h2 <= y1 || y2 > y1 + h1) && x2 + w2 < canvas.width + 100){
    ...
    if(keysDowned.indexOf('Numpad4') !== -1 && (x2 >= x1 + w1 || x2 + w2 <= x1 || y2 + h2 <= y1 || y2 > y1 + h1) && x2 > -100){
        if(spr2&&spr2Used < sprExpected){
            spr2Used++;
            immortality2 = true;
            immortality1 = true;
    ...
    if(keysDowned.indexOf('Numpad8') !== -1 && y2 >= canvas.height - p2ground - h2){
        let p2grDef = p2ground;
        let jumpAnim2 = setInterval(() => {
            isJumping2 = true;
            y2 -= jumpSpeed2 / 3;
            if (y2 <= canvas.height - jumpHeight2 - p2grDef || y2 < 0) {
                falling2 = true;
                isJumping2 = false;
    ...
    if(keysDowned.indexOf('Numpad5')!==-1){
        if(spr2Used < sprExpected) {
            spr2 = true;
        }
    }
    if(keysDowned.indexOf('Numpad0') !== -1){
...
let genBuffTime = setInterval(()=>{
    if(++generateBuffTimer >= _buffGenTime){
        generateBuffs();
        generateBuffTimer = 0;
    }
},1000);